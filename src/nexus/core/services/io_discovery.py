"""
Service for discovering IO declarations in plugins.
"""
...
class IODiscoveryService:
    """
    Service for discovering DataSource and DataSink declarations in plugins.
    """
    ...
    def _get_type_hints_cached(self, config_model: type) -> dict:
        """
    ...
    def discover_io_declarations(self, pipeline_steps: List[Dict[str, Any]], case_config: dict) -> Tuple[Dict[str, Any], Dict[str, Dict[str, DataSource]], Dict[str, Dict[str, DataSink]]]:
        """
        ...
        self.logger.debug(f"io_mapping: {io_mapping}")
        ...
        for step_config in pipeline_steps:
            ...
            try:
                ...
                type_hints = self._get_type_hints_cached(plugin_spec.config_model)
            except (NameError, TypeError) as e:
                self.logger.error(f"Could not resolve type hints for {plugin_name}'s config: {e}")
                continue
            ...
            for field_name, field_type in type_hints.items():
                ...
                for item in field_metadata:
                    if isinstance(item, DataSource):
                        ...
                        name = item.name
                        self.logger.debug(f"Found DataSource with name: {name}")
                        if name in base_data_sources:
                            self.logger.warning(f"Data source name '{name}' is declared by multiple plugins.")
                        
                        # Get path and handler from io_mapping
                        source_config = io_mapping.get(name, {})
                        self.logger.debug(f"Source config for '{name}': {source_config}")
                        path = source_config.get("path", "")
                        handler = source_config.get("handler", "")
                        self.logger.debug(f"Path for '{name}': {path}")
                        self.logger.debug(f"Handler for '{name}': {handler}")
                        ...
                        self.logger.debug(f"Added to base_data_sources: {name} -> {base_data_sources[name]}")
        ...
        if base_data_sources:
            self.logger.info(f"Discovered {len(base_data_sources)} data sources from plugin configs.")
