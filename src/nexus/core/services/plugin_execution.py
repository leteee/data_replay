"""
Service for executing plugins and handling their results.
"""
...
class PluginExecutionService:
    ...
    def execute_plugin(self, plugin_name: str, plugin_spec, config_object, 
        ...
        plugin_context = PluginContext(
            data_hub=data_hub, logger=self.logger,
    ...
    def handle_plugin_output(self, plugin_name: str, return_value: Any, 
        ...
        if not sinks_for_plugin:
            if return_value is not None:
                self.logger.debug(f"Plugin '{plugin_name}' returned a value but has no DataSink declared.")
            return

        if len(sinks_for_plugin) > 1:
            self.logger.warning(f"Plugin '{plugin_name}' has multiple DataSinks defined. Only one is supported per plugin. Using the first one found.")
        ...
        if return_value is None:
            ...
            self.logger.debug(f"Plugin '{plugin_name}' has a DataSink for field '{sink_field}' but returned None.")
            return

        self.logger.info(f"Plugin '{plugin_name}' produced output. Writing to sink: {sink_marker.name}")
        try:
