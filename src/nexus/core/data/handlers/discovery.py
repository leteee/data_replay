...
def discover_handlers(logger: Logger, project_root: Path = None, additional_paths: List[str] = None) -> None:
    ...
    # Discover built-in handlers from the new location
    try:
        # Import the entire nexus.handlers package which will trigger registration
        import nexus.handlers
        logger.debug("Successfully imported built-in handler modules via nexus.handlers package")
    except Exception as e:
        logger.error(f"Failed to import built-in handlers from nexus.handlers: {e}", exc_info=True)
    ...
    # Import the registry from the original location
