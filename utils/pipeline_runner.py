...
class PipelineRunner:
    ...
    def __init__(self, case_path: str, cli_args: dict = None):
        ...
        self.data_hub = {"results": {}} # Initialize the data bus
        ...
        print(f"产出物将保存在: {self.artifacts_dir}")

        self.plugin_map = self._find_plugins()
        print(f"发现 {len(self.plugin_map)} 个可用插件。")
    ...
    def _load_plugin_class(self, plugin_identifier: str):
        ...
        if "." in plugin_identifier:
            ...
            file_path = str(self.project_root / module_path.replace('.', '/') / f"{class_name.lower()}.py") # This is a bit of a hack
            # A better way would be to inspect the module, but this works for our convention.
    ...
    def run(self):
        ...
        print(f"开始执行流水线，共 {len(pipeline_steps)} 个步骤。")

        self.data_hub['context'] = {'case_path': str(self.case_path)}
        ...
        for i, step_config in enumerate(pipeline_steps):
            ...
            PluginClass, plugin_module_path = self._load_plugin_class(plugin_identifier)
            ...
            self.data_hub = plugin_instance.run(self.data_hub)
            ...
            if step_config.get("persist", False):
                ...
                if isinstance(self.data_hub.get("data"), pd.DataFrame):
                    self.data_hub["data"].to_parquet(output_path)
                    print(f"结果已持久化到: {output_path}")
                    self.data_hub["data"] = str(output_path)
                else:
                    print(f"警告: 插件 {plugin_identifier} 被设置为持久化，但其输出 data_hub['data'] 不是DataFrame。")
